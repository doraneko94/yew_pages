...
module ReVIEW
  ...
  class LATEXBuilder
    ...
    def inline_chapref(id)
      if in_appendix_mode?(id)
        chap = @book.chapter(id)
        caption = compile_inline(chap.title)
        appendix = appendix_label(id)
        "\\hyperref[chap:#{id}]{Appendix #{appendix}\"#{caption}\"}"
      else
        "\\hyperref[chap:#{id}]{\\ref{chap:#{id}}}"
      end
    end
    
    def in_appendix_mode?(id)
      id =~ /\A9\d-/
    end

    def appendix_label(id)
      m = /\A9(\d+)-/.match(id)
      unless m
        raise "Invalid appendix id format: #{id}"
      end
    
      number = "9#{m[1]}".to_i
      if number < 91
        raise "Appendix ID must start from 91: #{id}"
      end
    
      offset = number - 91
      if offset >= 26
        raise "Too many appendices (>26): #{id}"
      end
